...
private struct Mystify3DRepresentable: NSViewRepresentable {
    ...
    private func addBounceShape(to root: SCNNode, index: Int, seed: UInt64, rng: inout SplitMix64) {
        ...
        let freq = Float(Double.random(in: 0.4...0.9, using: &rng))
        let phaseX = Float(Double.random(in: 0...Float.pi * 2, using: &rng))
        let phaseY = Float(Double.random(in: 0...Float.pi * 2, using: &rng))
        let phaseZ = Float(Double.random(in: 0...Float.pi * 2, using: &rng))
