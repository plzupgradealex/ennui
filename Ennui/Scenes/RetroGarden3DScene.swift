...
private struct RetroGarden3DRepresentable: NSViewRepresentable {
    ...
    private func buildScene(_ scene: SCNScene, coord: Coordinator) {
        ...
        let spin = SCNAction.repeatForever(SCNAction.rotateBy(x: 0, y: 0, z: -CGFloat(Float.pi * 2), aroundTarget: .init(), duration: 8))
        let spinZ = SCNAction.repeatForever(SCNAction.rotate(by: CGFloat(Float.pi * 2), around: SCNVector3(0, 0, 1), duration: 8))
        ...
        let orbitRadii: [Float] = [0.3, 0.4, 0.35, 0.5, 0.3, 0.45]
        ...
        for b in 0..<6 {
            ...
            let yHeight = Float(1.0 + Double(b) * 0.2)
            let orbit = SCNAction.customAction(duration: dur) { node, elapsed in
                let angle = Float(elapsed / CGFloat(dur)) * Float.pi * 2
