...
struct MidnightMotelScene: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 60.0)) { tl in
            ...
            Canvas { ctx, size in
                ...
                drawAmbientFill(ctx: &ctx, w: w, h: h, t: t)
    ...
    // MARK: - Warm ambient room fill

    private func drawAmbientFill(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        // Soft overall warm light so the room doesn't feel like a void
        let breathe = sin(t * 0.25) * 0.01 + 1.0
        ctx.drawLayer { l in
            l.addFilter(.blur(radius: 100))
            // Warm pool from the lamp side
            l.fill(
                Ellipse().path(in: CGRect(x: w * 0.3, y: h * 0.15, width: w * 0.6, height: h * 0.7)),
                with: .color(Color(red: 0.80, green: 0.50, blue: 0.18).opacity(0.06 * breathe)))
            // Gentle glow near the bed area
            l.fill(
                Ellipse().path(in: CGRect(x: -w * 0.1, y: h * 0.3, width: w * 0.7, height: h * 0.5)),
                with: .color(Color(red: 0.70, green: 0.45, blue: 0.15).opacity(0.03 * breathe)))
        }
    }
    ...
    private func drawWalls(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        ...
        // Base wall color — warm wood paneling
        ctx.fill(
            ...
            with: .linearGradient(Gradient(colors: [
                ...
                Color(red: 0.19, green: 0.13, blue: 0.08),
        ...
        for i in 0..<14 {
            ...
            ctx.fill(
                ...
                with: .color(Color(red: 0.10 + grain, green: 0.07, blue: 0.05).opacity(0.4)))
            ...
            for _ in 0..<3 {
                ...
                ctx.fill(
                    ...
                    with: .color(Color(red: 0.24, green: 0.16, blue: 0.10).opacity(go)))
            }
        }
    ...
    private func drawWindow(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        ...
        ctx.fill(
            ...
            with: .color(Color(red: 0.14, green: 0.09, blue: 0.06)))

        // Night sky through window — deep blue
        ctx.fill(
    ...
    private func drawCurtains(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        ...
        ctx.drawLayer { l in
            ...
            left.closeSubpath()
            l.fill(left, with: .color(Color(red: 0.24, green: 0.18, blue: 0.13)))
        }
        ...
        ctx.drawLayer { l in
            ...
            right.closeSubpath()
            l.fill(right, with: .color(Color(red: 0.24, green: 0.18, blue: 0.13)))
        }

        // Curtain rod
        ctx.fill(
            Path(CGRect(x: wx - 15, y: wy - 3, width: ww + 30, height: 3)),
            with: .color(Color(red: 0.55, green: 0.40, blue: 0.20).opacity(0.5)))
    }
    ...
    private func drawVacancyGlow(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        ...
        // Soft amber-rose neon wash coming through window
        ctx.drawLayer { l in
            ...
            l.fill(Ellipse().path(in: glowRect),
                with: .color(Color(red: 0.95, green: 0.50, blue: 0.30).opacity(baseGlow * 0.9)))
        }

        // Neon glow on wall beneath window — warm amber spill
        ctx.drawLayer { l in
            ...
            l.fill(Ellipse().path(in: wallGlow),
                with: .color(Color(red: 0.85, green: 0.45, blue: 0.22).opacity(baseGlow * 0.4)))
        }
        ...
        ctx.drawLayer { l in
            ...
            for (i, cw) in [8.0, 7.0, 9.0, 7.0, 8.0, 8.0, 8.0].enumerated() {
                ...
                l.fill(
                    ...
                    with: .color(Color(red: 1.0, green: 0.55, blue: 0.30).opacity(baseGlow * 1.2)))
            }
        }
    }
    ...
    private func drawBed(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        ...
        // Bed frame — warm dark wood
        ctx.fill(
            Path(CGRect(x: bx - 3, y: by - 2, width: bw + 6, height: bh + 5)),
            with: .color(Color(red: 0.16, green: 0.10, blue: 0.07)))
    ...
    private func drawNightstand(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        ...
        ctx.fill(
            ...
            with: .color(Color(red: 0.17, green: 0.11, blue: 0.07)))
    ...
    private func drawLamp(ctx: inout GraphicsContext, w: Double, h: Double, t: Double) {
        ...
        // Lamp glow — warm amber puddle of light (dominant room illumination)
        ctx.drawLayer { l in
            l.addFilter(.blur(radius: 70))
            let glowR = 130.0 * breathe
            l.fill(
                Ellipse().path(in: CGRect(x: lx - glowR, y: ly - glowR * 0.7, width: glowR * 2, height: glowR * 1.4)),
                with: .radialGradient(
                    Gradient(colors: [
                        Color(red: 0.95, green: 0.70, blue: 0.25).opacity(0.40 * breathe),
                        Color(red: 0.90, green: 0.55, blue: 0.18).opacity(0.18),
                        Color(red: 0.70, green: 0.40, blue: 0.12).opacity(0.06),
                        .clear
                    ]),
        ...
        shade.addLine(to: CGPoint(x: lx - botW/2, y: shadeBot))
        shade.closeSubpath()
        ctx.fill(shade, with: .color(Color(red: 0.60, green: 0.42, blue: 0.18).opacity(0.6)))

        // Inner glow through shade
        ctx.drawLayer { l in
            l.addFilter(.blur(radius: 6))
            l.fill(shade, with: .color(Color(red: 0.90, green: 0.60, blue: 0.20).opacity(0.25 * breathe)))
        }
    ...
    private func drawCarpet(ctx: inout GraphicsContext, w: Double, h: Double) {
        ...
        for _ in 0..<80 {
            ...
            ctx.fill(
                ...
                with: .color(Color(red: 0.20, green: 0.13, blue: 0.08).opacity(co)))
        }
    }
