    ...
    private func buildScene(_ scene: SCNScene, coord: Coordinator) {
        ...
        for _ in 0..<10 {
            ...
            for w in 0..<winCount {
                ...
                let wy = Float(Double.random(in: -Float(bh)/2+0.3...Float(bh)/2-0.3, using: &rng))
                let wx = Float(Double.random(in: -Float(bw)/2+0.2...Float(bw)/2-0.2, using: &rng))
                wNode.position = SCNVector3(wx, wy, Float(bw * 0.4) + 0.01)
        ...
        dust.emitterShape = SCNBox(width: 18, height: 0, length: 14)
